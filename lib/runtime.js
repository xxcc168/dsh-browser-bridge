...
export class BridgeClient {
  constructor(options={}) {
    ...
    this.identities=new Map();this.claims=new Map();
  ...
  async claim(tabId,identity,options) {
    ...
    // A cache entry keeps renewal bookkeeping, never permission to operate.
    // Explicit old credentials remain strict; only implicit ownership can recover.
    try {
      const data=await this.sessionState(options.sessionId?"status":"ensure",tabId,{...options,...identity});
      if(!data.canWrite)throw failure(data.reason || "UPGRADE_REQUIRED","当前任务没有经扩展确认的控制权",{details:data});
      const entry=Object.assign(this.claims.get(key) || {},data,identity,{activity:Date.now()});
      this.claims.set(key,entry);return entry;
    }catch(err){this.claims.delete(key);throw err;}
  }
  async sessionState(action,tabId,options) {
    try {
      return await this.request("POST","/api/sessions",{action,tabId,...(options.sessionId?{sessionId:options.sessionId}:{})},
        {...options,requestId:undefined,sessionId:undefined});
    }catch(err) {
      if(!["INVALID_ARGUMENT","NOT_FOUND"].includes(err.code))throw err;
      const ready=await this.request("GET","/api/status",undefined,{...options,requestId:undefined,sessionId:undefined});
      if(ready.capabilities?.includes("atomic-session-v1"))throw err;
      return {tabId,connected:ready.connected,verified:false,observedAt:Date.now(),instanceId:ready.instanceId,
        versions:{bridge:ready.version,extension:ready.extension?.version || null,expectedExtension:manifest.version},
        state:"UPGRADE_REQUIRED",reason:"UPGRADE_REQUIRED",canRead:false,canWrite:false,reacquireAllowed:false,
        recovery:{action:"restart_bridge",message:"重启 bridge 并重载扩展，再重连 MCP。",automaticRetryAllowed:false}};
    }
  }
  async renewControls() {
    ...
    for(const [key,l] of this.claims) {
      if(Date.now()-l.activity>=180000 && !l.running){this.claims.delete(key);continue;}
    ...
    for(const entries of groups.values()) {
      try {
        ...
        for(const l of entries) {
          if(data.renewed.includes(l.sessionId))l.expiresAt=Date.now()+115000;
  ...
  async run(name,args={},options={}) {
    ...
    if(name==="browser_status") {
      ...
      return text({...data,adapterVersion:manifest.version,versionsMatch:data.version===manifest.version && data.extension?.version===manifest.version,
        sessionApiAvailable:!!(data.capabilities?.includes("atomic-session-v1") && data.extension?.capabilities?.includes("atomic-session-v1"))});
    }
    ...
    if(["browser_session_status","browser_ensure_session"].includes(name)) {
      const data=await this.sessionState(name==="browser_session_status"?"status":"ensure",args.tabId,context);
      if(name==="browser_ensure_session" && data.canWrite) {
        const key=identity.owner+":"+args.tabId;
        this.claims.set(key,Object.assign(this.claims.get(key) || {},data,identity,{activity:Date.now()}));
      }
      else if(!data.canWrite)this.claims.delete(identity.owner+":"+args.tabId);
      return text({...data,versions:{...data.versions,adapter:manifest.version,bridgeMatches:data.versions?.bridge===manifest.version}});
    }
    if(args.tabId===undefined || name==="browser_session") {
      const ready=await call("GET","/api/status",undefined,{requestId:undefined});
      if(ready.protocolVersion!==3 || ready.extension?.protocolVersion!==3 || !ready.capabilities?.includes("atomic-session-v1") || !ready.extension?.capabilities?.includes("atomic-session-v1"))
        throw failure("UPGRADE_REQUIRED","需要重启新版 bridge 并重载 Chrome 扩展（atomic-session-v1）",{details:{adapterVersion:manifest.version,bridgeVersion:ready.version,extensionVersion:ready.extension?.version}});
    }
